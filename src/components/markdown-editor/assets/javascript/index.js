...
export const insertInline = (vm, textarea, inserArr) => {
    ...
    if (start === end) {
        vm.content = insertContent(
            content,
            inserArr.join(deftext),
            start,
            false
        );
...
export const insertBlock = (vm, textarea, insertArr, increment = false) => {
    ...
    if (start === end) {
        ...
        const index2 =
            newlineIndex === -1 ? content.length : start + newlineIndex;
        ...
        // 按start,end分三个片段处理
        vm.content = replaceWithfragment(
            content,
            insertArr,
            start,
            end,
 ...
 * @params {increment} 自增数字
 */
export const replaceWithfragment = (content, inserArr, start, end, increment = false) => {
